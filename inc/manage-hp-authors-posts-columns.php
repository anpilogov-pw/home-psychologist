...
add_filter('manage_hp_authors_posts_columns', function ($columns) {
...
add_filter('manage_edit-hp_authors_sortable_columns', function ($columns) {
...
add_action('pre_get_posts', function ($query) {
	...
	if ($query->get('orderby') === 'acf_field_name') {
		$query->set('meta_key', 'hp_author_name');
...
add_action('manage_hp_authors_posts_custom_column', function ($column, $post_id) {
	if ($column === 'acf_field_name') {
		$name = get_field('hp_author_name', $post_id);
		$secondname = get_field('hp_author_secondname', $post_id);
		$link = get_edit_post_link($post_id);
		echo '<a href="' . esc_url($link) . '">' . esc_html($name) . "&nbsp;" . esc_html($secondname) . '</a>';
	}
	if ($column === 'acf_field_avatar') {
		$image_url = get_field('hp_author_avatar', $post_id);
		if ($image_url) {
...
add_action('acf/save_post', function ($post_id) {
	if (get_post_type($post_id) !== 'hp_authors' || is_numeric($post_id) === false) {
		return;
	}
